...
- **加深后的 module 以某个概念命名，但该概念不在 `CONTEXT.md` 中？** 将该术语加入 `CONTEXT.md`，遵循和 `/grill-with-docs` 相同的纪律（见 [CONTEXT-FORMAT.md](../grill-with-docs/CONTEXT-FORMAT.md)）。如果文件不存在，按需创建。
- **对话中收紧了一个模糊术语？** 立即更新 `CONTEXT.md`。
- **用户基于关键理由拒绝了候选项？** 提议记录 ADR，表述为：“要不要把这个记录成 ADR，避免未来架构评审再次提出同一个建议？” 只有当这个理由确实能帮助未来探索者避免重复建议时才提出；临时理由（“现在不值得”）和显而易见的理由跳过。见 [ADR-FORMAT.md](../grill-with-docs/ADR-FORMAT.md)。
