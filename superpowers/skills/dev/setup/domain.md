...
如果这些文件不存在，**静默继续**。不要特别指出它们缺失，也不要一开始就建议创建它们。生成这些文档的 skill（`/grill-with-docs`）会在术语或决策真正被澄清时按需创建。
...
如果需要的概念还不在术语表中，这是一个信号：要么你正在发明项目没有使用的语言，需要重新考虑；要么这里确实有空缺，应该记下来交给 `/grill-with-docs` 处理。
