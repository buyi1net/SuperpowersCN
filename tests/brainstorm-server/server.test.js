...
const SERVER_PATH = path.join(__dirname, '../../skills/dev/brainstorming/scripts/server.cjs');
  ...
  try {
    ...
    await test('helper.js defines required APIs', () => {
      const helperContent = fs.readFileSync(
        path.join(__dirname, '../../skills/dev/brainstorming/scripts/helper.js'), 'utf-8'
      );
    ...
    await test('frame template has required structure', () => {
      const template = fs.readFileSync(
        path.join(__dirname, '../../skills/dev/brainstorming/scripts/frame-template.html'), 'utf-8'
      );
