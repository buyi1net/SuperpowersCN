...
- 命令式语言：`你必须（YOU MUST）`、`绝不（Never）`、`始终（Always）`。
- 不可协商框架：`无例外（No exceptions）`。
- 减少决策疲劳和合理化。
...
```markdown
✅ 每次找到 skill 时，你必须声明："我正在使用 [Skill 名称]"
❌ 可以考虑让你的搭档知道你正在用哪个 skill。
```
